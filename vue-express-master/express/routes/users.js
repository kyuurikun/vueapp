...
router.post('/add', function(req, res, next) {
    ...
    db.query(`SELECT username FROM user where user_name='${username}'`,function(err,rows){
        console.log(err,rows)
        if(rows && rows.length>0){
            data={data:'',meta:{code:'500',message:'用户名存在'}}
            res.send(data)
        }else{
            db.query(`INSERT INTO user (user_name,login_password) VALUES ('${username}','${password}')`,function(err,rows){
...
router.post('/login', function(req, res, next) {
    ...
    db.query(`SELECT * FROM user where user_name='${username}'`,function(err,rows){
        if(rows && rows.length>0 && rows[0].login_password == password){
            // 输出签发的 Token
